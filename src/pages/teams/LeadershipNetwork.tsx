        ...
        <div className="container mx-auto px-6 max-w-7xl space-y-12">
          {sectionsToShow.map((section) => (
            <div key={section.id}>
              ...
              {section.people && (
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-y-6 md:gap-y-8 gap-x-0">
                  {section.people.map((p, i) => (
                    <div key={`${section.id}-${p.name}-${i}`} className="rounded-xl p-4 sm:p-6 hover:bg-gray-50 transition-colors">
